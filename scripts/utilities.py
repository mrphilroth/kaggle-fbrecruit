...
def read_graph_and_inverse(file_name):
    """
    Reads a sparsely represented directed graph into a dictionary
    """
    ...
    for nodes in edges_generator(file_name):
        for node in nodes:
            initialize_node(node)
        graph[nodes[0]].append(nodes[1])
        graph_inverse[nodes[1]].append(nodes[0])

    return graph, graph_inverse

def read_graph_and_inverse_skip_test(file_name, test_nodes):
    """
    Reads a sparsely represented directed graph into a dictionary
    """
    ...
    for nodes in edges_generator(file_name):
        if nodes[0] in test_nodes or nodes[1] in test_nodes: continue
        for node in nodes:
            initialize_node(node)
        graph[nodes[0]].append(nodes[1])
        graph_inverse[nodes[1]].append(nodes[0])
